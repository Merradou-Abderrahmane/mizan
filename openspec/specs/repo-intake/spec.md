# repo-intake Specification

## Purpose

The repo intake service is the synchronous bridge between the runner CLI
(`runner-cli` spec) and the domain model (`domain-model` spec): it takes a
Git URL or local path, shallow-clones (URL only), invokes the runner
**unchanged** (R2), parses the JSON report, persists a `Run` with the full
`runner_report_json` blob plus a linked `StudentRepo`, and deletes the temp
clone.

Per the operator's Option X decision (see the archived
`repo-intake` change), the service creates **zero `Evidence` rows**: runner
structural checks are input artifacts, not per-competence R3 findings, and
live in the `runner_report_json` blob on the `Run`. The `evidence` table stays
strictly per-competence (`competence_id` non-nullable, FK RESTRICT) as the
`domain-model` spec defines it. A `check_results` table is NOT added yet
(YAGNI — no consumer needs runner checks queryable in SQL; it can be added
additively when a real feature requires it, without touching R3).

Hard rules respected: R2 (runner called as-is), R3 (no Evidence rows from the
runner; per-competence Evidence is the LLM Pass 1 output, a later change), R4
(`operator_persona` stored on `StudentRepo` and `$hidden`, never passed to the
runner subprocess, never in `Evidence`), R5 (boring — one service, one
subprocess call, one parse, one transaction, one cleanup).
## Requirements
